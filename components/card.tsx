  ...
  return (
    <>
      ...
      <div className={cn('card',
          'relative overflow-hidden flex h-[140px] rounded-lg bg-light shadow-card',
          'md:h-[146px] hover:cursor-pointer'
      )} onClick={toggleCard}>
        <div className="relative z-1 grid w-[34.1246290801%] pl-5 pt-2 pb-3 text-[18px] font-bold">
          <h2 className={cn('text-dark')}>
            <span>{ name }</span>
          </h2>
        ...
        <div className={cn('card-overlay',
          ...
          <div style={{ background: 'linear-gradient(270deg, #B33327 0.15%, #D93E30 100%)' }} className={cn(
            ...
            <button onClick={toggleCard} className={cn(
              'absolute top-[15px] left-[14px]',
              'md:-top-[45px] md:left-auto md:right-0'
            )}>
              <SvgClose />
            </button>
            <div className={cn('content',
              'grid gap-y-5',
              'md:grid-cols-2 md:transform md:h-full'
            )}>
              <div style={{ background: 'linear-gradient(180deg, #732119 42.19%, #D93E30 100%)'}} 
                className={cn(
                  'absolute -z-1 inset-y-0 inset-x-0 top-[350px] rounded-xl md:top-0'
              )}/>

              <h2 className="relative md:hidden text-light font-bold text-[36px] text-center">{ name }</h2>
              <div className={cn(
                'relative grid gap-y-5'
              )}>
                ...
                <div className={cn(
                  ...
                  'md:fixed md:right-[50%] md:bottom-[20px] md:pr-4',
                )}>
                  <div className={cn('card-info-1',
                    'flex gap-x-4 items-center',
                    'md:fixed md:top-[18px] md:right-[16px]',
                  )}>
                    <span className="rounded-[50%] bg-warning w-[41px] h-[41px] flex items-center justify-center text-[16px] text-black md:order-1">{number}</span>
              ...
              <div className={cn(
                'relative grid gap-y-5 content-start',
                'md:pr-4 md:pb-5 md:pt-6 md:pl-3'
              )}>
                ...
                <div className="info-box p-5 bg-light-3 rounded-xl grid gap-y-3 shadow-textfield md:order-1">
                  ...
                  <ul className='grid grid-cols-2 gap-x-3 md:gap-x-2'>
                    <li className="grid gap-y-2 content-start md:gap-y-1">
                      ...
                      {attacks.fast.map(({ name, type }: any, index) => (
                    ...
                    <li className="grid gap-y-2 content-start md:gap-y-1">
                      ...
                      {attacks.special.map(({ name, type }: any, index) => (
                        <div key={index} className="flex gap-x-2">
                          <span className="text-[14px] md:text-[12px]">{ name }</span>
                          <Pills types={[type]} />
                        </div>
                ...
                <div className="info-box p-5 bg-light-3 rounded-xl grid gap-y-3 shadow-textfield">
                  <ul className='grid grid-cols-2 gap-x-3'>
                    ...
                    <li className="grid gap-y-1 content-start">
                      <h4 className="text-[14px]">Max HP: { maxCP }</h4>
                      <div className="bg-light rounded-md h-[8px] relative overflow-hidden">
                        <div 
